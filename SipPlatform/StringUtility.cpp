 ...
 * @brief 문자열에 포함된 문자열을 수정한다.
 * @param strCallId 문자열
 * @param cBefore		수정 대상 문자
 * @param cAfter		수정 대상 문자를 수정할 문자
 */
void ReplaceString( std::string & strCallId, char cBefore, char cAfter )
{
	char szBefore[2], szAfter[2];

	snprintf( szBefore, sizeof(szBefore), "%c", cBefore );
	snprintf( szAfter, sizeof(szAfter), "%c", cAfter );

	ReplaceString( strCallId, szBefore, szAfter );
}
